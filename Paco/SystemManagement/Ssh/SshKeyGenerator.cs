// Credits to: ShawInnes/SshKeyGenerator (https://github.com/ShawInnes/SshKeyGenerator)
...
namespace Paco.SystemManagement.Ssh
{
    public class SshKeyGenerator : IDisposable
    {
        ...
        {
            if (_csp.PublicOnly) throw new ArgumentException("CSP does not contain a private key", nameof(_csp));
            var parameters = _csp.ExportParameters(true);
        ...
        public string ToRfcPublicKey(string comment)
        {
            ...
            byte[] n = _csp.ExportParameters(false).Modulus;
            byte[] e = _csp.ExportParameters(false).Exponent;
            string buffer64;
        ...
        {
            var parameters = _csp.ExportParameters(false);
            using (var stream = new MemoryStream())
            {
                var writer = new BinaryWriter(stream);
                writer.Write((byte) 0x30); // SEQUENCE
