...
namespace Paco.SystemManagement.FreeBsd.Commands
{
    public class CheckVersion
    {
        ...
        {
            var running = client.CreateCommand("sudo freebsd-version").Execute().Replace("\n", "");

            var runningVersionOnly = client.CreateCommand("sudo freebsd-version | cut -d \"-\" -f 1").Execute().Replace("\n", "");
